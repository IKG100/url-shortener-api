...
public class UserValidator {
    ...
    public void validate(String identifier, String password) throws ValidationException {
        ...
        // Validate identifier (either login or email)
        if (identifierType == AuthIdentifierType.LOGIN) {
            errors.append(validateLogin(identifier));
        } else if (identifierType == AuthIdentifierType.EMAIL) {
            errors.append(validateEmail(identifier));
        } else {
            errors.append(IDENTIFIER_EMPTY_MESSAGE);
        }
    ...
    private String validateLogin(String login) {
        ...
        if (!login.matches("^[a-zA-Z0-9]+$") || (login.length() < MIN_LOGIN_LENGTH
